...
| **Yellow** | **Evidence and facts**: execution, dating, delivery of notices/waivers/consents; **live COI** (including **§228** permissibility); cap table vs quorum stories; **record-date** math vs bylaws for meeting-path corps. **Record depth**: routine ratification tone—**thin** for contested M&A or self-dealing. **Board pattern**: Hippo, Ritual Growth, and TeamBoost are minuted as **sole-director** boards in the registry configuration—**internally coherent** if facts match; diligence will ask for **why** ordering or structure differs by company. |
| **Red (integrity)** | **None** identified solely from template logic for the registry companies named above; **RED** would require **fact** mismatches (e.g. minutes show a second director at a meeting when only one director was actually seated—confirm against **actual** board composition and dates). |
...
| Hippo / Ritual / TB | **Board noted** vs **Sole Director** on §228 cross-ref at AGM | Consistency | Practice | **Low** (mitigated) | Generator uses **Sole Director** for sole-director AGMs; if you later configure a multi-director board, cross-refs should follow the same voice as the roll call. |
| Hippo / Ritual / TB | **Organizational** meeting | Facts vs narrative | §141; org practice | **Low–Med** | Org minutes should match **who** was actually present and authorized to act on the org date. |
| Hippo / Ritual / TB | **First** board meeting in the series | Sole director narrative | §141 | **Med** if facts differ | Confirm the **first** minuted board meeting matches **actual** board size and any director appointments on the dates used. |
| TeamBoost | **Q1** after organizational | Director roster | §141 | **Low** if facts match | Confirm roster and appointment timing vs **stock ledger** and any **written consents**. |
...
4. **Review pack / samples:** After regeneration, run **`scripts/build_review_pack.py`** if you rely on **`review_pack/`** (repo root) snapshots; use **`--write-samples`** for **`generated/<safe>/samples/`** PDF/CSV samples (master all-companies book/CSV stay under **`generated/all_companies/`**).
...
- **Output paths:** Under **`generated/`**, only **`generated/<safe>/`** registry trees plus **`generated/all_companies/`** for the master book and combined CSV. Per company: **`meetings/`**, **`cap_tables/`**, **`stock_ledgers/`**, optional **`samples/`**, compiled **`<safe>_all_meetings_book.*`** at the company root, and optional **`stock_certificates/`**; **standalone** equity/domestication resolutions under **`meetings/`**.  
...
- **Output tree (flat meetings + per-company samples):** Loose minutes under **`generated/<safe>/meetings/`**; compiled **`<safe>_all_meetings_book`** at **`generated/<safe>/`** root; cap table / stock ledger under **`cap_tables/`** and **`stock_ledgers/`**; **`--write-samples`** PDF/CSV under **`generated/<safe>/samples/`**; master all-companies book + combined CSV under **`generated/all_companies/`**.
