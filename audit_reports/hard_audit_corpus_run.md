...
**Last execution note:** After a full run, **`generated/` `.docx` count** and **`audit_text/*.txt` count** should match (recent full run: **253** / **253**). The extractor **deletes** stale `.txt` files that no longer have a matching `.docx`.
