...
**Last execution note:** After a full run, **`generated/` `.docx` count** and **`audit_text/*.txt` count** should match (recent full run with **`--write-examples`**: **276** / **276**; without examples, lower). The extractor **deletes** stale `.txt` files that no longer have a matching `.docx`.
