...
1. After any template or `companies` change: regenerate and mirror extracts using the commands in **README.md** (typically `--output-root generated --extract-audit-text`, plus optional `--write-samples` / `--write-master-book` as needed).
